...
We shall close out the discussion on geometric deep learning and GNNs by following up on \Cref{sec:janossy-pooling} and going into a bit more detail about how self-attention. While our geometric deep learning based perspective is still valid, there are several details that affect the implementation, although the form of self-attention remains the same as we have discussed in earlier sections.
...
The \highlight[comment=write why softmax is used]{softmax} function can be sensitive to very large input values. These kill the gradient, slow down learning, or cause it to stop altogether. Since the average value of the dot product increases with embedding dimension, $d$.
\begin{equation}
