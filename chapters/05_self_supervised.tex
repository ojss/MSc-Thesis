...
We calculate the pairwise cosine similarity between each augmented image in a mini-batch using \Cref{eqn:simclr-cosine-sim}. \cref{fig:positive-pairs} shows an example of what positive pairs might look like. In the ideal case, the similarity between all \textit{positive} pairs must be high and low between all \textit{negative} pairs.
...
You can choose $\tau$ to be any value; higher values of $\tau$ will lead to a \textquote{softer} output distribution of probabilities, for example, $\left[0.2,0.2,0.6\right]$.
As $\tau$ approaches $0$, it will lead to a distribution \textquote{sharper}, for example, $\left[0.01,0.01,0.98\right]$.
