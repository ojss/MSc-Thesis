...
Then a question naturally arises, how do we create valid tasks (see \Cref{sec:formalising-fsl}) if we do not have access to the true labels? This is an important question, consider \Cref{fig:fsl-tasks} where we illustrate (\nwks{2}{4}) shots, we see that each of the $2$ classes have exactly $4$ samples. Tasks like this would be taxing to create without the labels. To overcome this there are a few creative methods that we shall discuss in the coming sections.
